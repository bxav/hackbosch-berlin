...
[ -d .tmp ] || mkdir .tmp
docker save -o .tmp/image.tar azureiotpcs/pcs-config-dotnet:$PROJECT_NAME
...
cd /app;
sed 's/azureiotpcs\/pcs-config-dotnet:1.0.0-preview.3/azureiotpcs\/pcs-config-dotnet:$PROJECT_NAME/' docker-compose.yml | sudo tee docker-compose.yml;
sudo ./start.sh
EOF
